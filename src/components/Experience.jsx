...
const Experience = () => {
  return (
    <section id="experience" className="py-20 px-6 md:px-12 max-w-4xl mx-auto border-t border-white/5">
        ...
        {experience.map((job, index) => {
          ...
          return (
            <div key={job.id} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
              ...
              {/* Icon */}
              <div className="flex items-center bg-white justify-center w-3 h-3 rounded-full border-2 border-white/20 bg-dark shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 shadow-lg z-10 text-white">
              </div>
              ...
              <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-6 rounded-xl bg-card border border-white/5 hover:border-white/20 transition-all shadow-md group-hover:-translate-y-1">
                <div className="flex flex-col mb-2">
                  ...
                  <div className="text-gray-400 font-mono text-sm tracking-tight mt-1">{job.company}</div>
                </div>
                <time className="text-sm font-mono text-gray-500 mb-4 block">{job.duration}</time>
